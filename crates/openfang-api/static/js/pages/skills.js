...
function skillsPage() {
  return {
    ...
    runtimeBadge: function (rt) {
      ...
      if (r === "python" || r === "py")
        return { text: "PY", cls: "runtime-badge-py" };
      if (r === "node" || r === "nodejs" || r === "js" || r === "javascript")
        return { text: "JS", cls: "runtime-badge-js" };
      if (r === "wasm" || r === "webassembly")
        return { text: "WASM", cls: "runtime-badge-wasm" };
      if (r === "prompt_only" || r === "prompt" || r === "promptonly")
        return { text: "PROMPT", cls: "runtime-badge-prompt" };
      return {
        text: r.toUpperCase().substring(0, 4),
        cls: "runtime-badge-prompt",
      };
    },
    ...
    sourceBadge: function (source) {
      ...
      switch (source.type) {
        case "clawhub":
          return { text: "ClawHub", cls: "badge-info" };
        case "openclaw":
          return { text: "OpenClaw", cls: "badge-info" };
        case "bundled":
          return { text: "Built-in", cls: "badge-success" };
        default:
          return { text: "Local", cls: "badge-dim" };
      }
    },
    ...
    async searchClawHub() {
      ...
      try {
        var data = await OpenFangAPI.get(
          "/api/clawhub/search?q=" +
            encodeURIComponent(this.clawhubSearch.trim()) +
            "&limit=20"
        );
    ...
    async browseClawHub(sort) {
      ...
      try {
        var data = await OpenFangAPI.get(
          "/api/clawhub/browse?pageSize=20&page=1"
        );
        this.clawhubBrowseResults = data.data.skills || [];
    ...
    async loadMoreClawHub() {
      ...
      try {
        ...
        this.clawhubBrowseResults = this.clawhubBrowseResults.concat(
          data.data.skills || []
        );
        this.clawhubNextCursor = data.next_cursor || null;
      } catch (e) {
    ...
    async showSkillDetail(slug) {
      ...
      try {
        var data = await OpenFangAPI.get(
          "/api/clawhub/skill/" + encodeURIComponent(slug)
        );
    ...
    async viewSkillCode(slug) {
      ...
      try {
        var data = await OpenFangAPI.get(
          "/api/clawhub/skill/" + encodeURIComponent(slug) + "/code"
        );
    ...
    async installFromClawHub(slug) {
      ...
      this.installResult = null;
      try {
        var data = await OpenFangAPI.post("/api/clawhub/install", {
          slug: slug,
        });
        ...
        if (data.warnings && data.warnings.length > 0) {
          ...
          OpenFangToast.success(
            'Skill "' + data.name + '" installed successfully'
          );
        }
    ...
    quickStartSkills: [
      {
        ...
        description:
          "Adds code review best practices and checklist to agent context.",
        prompt_context:
          "You are an expert code reviewer. When reviewing code:\n1. Check for bugs and logic errors\n2. Evaluate code style and readability\n3. Look for security vulnerabilities\n4. Suggest performance improvements\n5. Verify error handling\n6. Check test coverage",
      },
      {
        name: "writing-style",
        description: "Configurable writing style guide for content generation.",
        prompt_context:
      ...
      },
      {
        ...
        description: "OWASP-aligned security review checklist.",
        prompt_context:
    ...
    isSkillInstalled: function (slug) {
      return this.skills.some(function (s) {
        return (
          s.source && s.source.type === "clawhub" && s.source.slug === slug
        );
      });
    },
